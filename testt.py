...
# ═══════════════════════════════════════════════════════════════════════════════
# ║  БЛОК 0. ИНФРАСТРУКТУРА: запуск сервера, утилиты, отчётность               ║
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
# ║  БЛОК 0. ОТЧЁТНОСТЬ: сбор результатов, таблицы скорости и производительности ║
# ═══════════════════════════════════════════════════════════════════════════════

class Report:
    def __init__(self):
        self.lines, self.p, self.f, self.w, self.defects, self.speed = [], 0, 0, 0, [], []
    ...
    def speed_table(self):
        ...
        self.add(); self.add("   СКОРОСТЬ API:")
        self.add("   " + "-"*55); self.add(f"   {'Запрос':<30} {'Время':<12} {'Статус':<10}")
        self.add("   " + "-"*55)
        times = []
        for name, _, ms, st in self.speed:
            times.append(ms); self.add(f"   {name:<30} {ms:>7.1f} мс    {st}")
        self.add("   " + "-"*55)
        if times:
            self.add(f"   Среднее: {sum(times)/len(times):.1f} мс | Мин: {min(times):.1f} | Макс: {max(times):.1f}")

    def perf_table(self, records):
        self.add(); self.add("   ПРОИЗВОДИТЕЛЬНОСТЬ:")
        self.add("   " + "-"*55)
        self.add(f"   {'Тест':<25} {'RPS':<10} {'Время':<10} {'Статус':<10}")
        self.add("   " + "-"*55)
        for name, rps, elapsed, status in records:
            self.add(f"   {name:<25} {rps:>7.1f}      {elapsed:>7.1f} мс   {status}")
        self.add("   " + "-"*55)

    def summary(self):
        total = self.p + self.f + self.w
        self.add(); self.add("="*67); self.add("ИТОГИ"); self.add("="*67)
        self.add(f"   Всего: {total} | [OK] {self.p} | [WARN] {self.w} | [FAIL] {self.f}")
        if self.defects:
            self.add("   ДЕФЕКТЫ:")
            for i, (n, d, r) in enumerate(self.defects, 1): self.add(f"   {i}. {n}: {d} → {r}")
        self.add("   Данные восстановлены"); self.add("="*67)

    def save(self):
        with open(REPORT, 'w', encoding='utf-8') as f: f.write('\n'.join(self.lines))
        log(f"Отчёт: {os.path.abspath(REPORT)}", C.OK)
...
# ═══════════════════════════════════════════════════════════════════════════════
# ║  БЛОК 1. СИСТЕМНЫЕ ТЕСТЫ: работоспособность всего приложения целиком       ║
# ║  Проверяем: сервер жив, товары видны, заказ создаётся, история работает    ║
# ═══════════════════════════════════════════════════════════════════════════════
...
def t03_detail():
    ...
    r = measure("Детали", lambda: requests.get(f"{BASE_URL}{API}/products/{pid}", timeout=5))
    assert "name" in r.json()["product"]; rep.test("1.3", "Детали товара", "PASS", "данные корректны")

def t04_404():
    r = measure("404", lambda: requests.get(f"{BASE_URL}{API}/products/xxx", timeout=5))
    assert r.status_code == 404; rep.test("1.4", "Ошибка 404", "PASS", "несуществующий ID")

def t05_create():
    item = get_product()
    r = measure("Создание", lambda: create_order("Иван", item))
...
def t07_incomplete():
    r = measure("Неполный", lambda: requests.post(f"{BASE_URL}{API}/order", json={"customer": {"name": "Иван"}, "products": []}, timeout=5))
    assert r.status_code == 400; rep.test("1.7", "Неполные данные", "PASS", "отклонение")

def t08_empty():
    r = measure("Пустое", lambda: requests.post(f"{BASE_URL}{API}/order", json=None, timeout=5))
    assert r.status_code == 400; rep.test("1.8", "Пустой заказ", "PASS", "отклонение")

def t09_history():
    r = measure("История", lambda: requests.get(f"{BASE_URL}{API}/orders", timeout=5))
...
# ═══════════════════════════════════════════════════════════════════════════════
# ║  БЛОК 2. ВАЛИДАЦИЯ: отклоняет ли система НЕПРАВИЛЬНЫЕ данные             ║
# ║  Проверяем: email, отрицательная цена, XSS, SQL-инъекции                   ║
# ═══════════════════════════════════════════════════════════════════════════════

def t20_email():
    item = get_product()
    r = measure("Email", lambda: create_order("E", item, {"customer": {"name": "T", "email": "не-email", "phone": "+7"}, "products": [{"id": str(item["id"]), "name": item["name"], "price": item["price"], "quantity": 1}], "total_amount": item["price"], "payment_method": "card"}))
...
def t21_negative():
    item = get_product()
    r = measure("Отриц.цена", lambda: create_order("N", item, {"customer": {"name": "T", "email": "t@t.com", "phone": "+7"}, "products": [{"id": str(item["id"]), "name": item["name"], "price": -1000, "quantity": 1}], "total_amount": -1000, "payment_method": "card"}))
...
def t22_xss():
    item = get_product()
    r = measure("XSS", lambda: create_order("X", item, {"customer": {"name": "X", "email": "x@x.com", "phone": "+7"}, "products": [{"id": str(item["id"]), "name": item["name"], "price": item["price"], "quantity": 1}], "total_amount": item["price"], "payment_method": "card", "comment": "<script>alert('xss')</script>"}))
    assert "<script>" not in r.text or "alert(" not in r.text; rep.test("2.3", "XSS", "PASS", "скрипт обезврежен")

def t23_sql():
    item = get_product()
    r = measure("SQL", lambda: create_order("S", item, {"customer": {"name": "'; DROP TABLE orders; --", "email": "s@s.com", "phone": "+7"}, "products": [{"id": str(item["id"]), "name": item["name"], "price": item["price"], "quantity": 1}], "total_amount": item["price"], "payment_method": "card"}))
    assert requests.get(f"{BASE_URL}{API}/orders", timeout=5).status_code == 200; rep.test("2.4", "SQL", "PASS", "данные не повреждены")


# ═══════════════════════════════════════════════════════════════════════════════
# ║  БЛОК 3. ВЕРИФИКАЦИЯ: правильно ли работают расчёты, форматы, сохранение   ║
# ║  Проверяем: формат ID, формат даты, математика, согласованность данных     ║
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
# ║  БЛОК 4. ГРАНИЧНЫЕ ЗНАЧЕНИЯ: поведение на "краях" допустимого             ║
# ║  Проверяем: 0 штук, 10000 штук, пустой заказ, длинное имя                  ║
# ═══════════════════════════════════════════════════════════════════════════════

def t40_zero():
    item = get_product()
    r = measure("Кол-во=0", lambda: create_order("Z", item, {"customer": {"name": "Z", "email": "z@z.com", "phone": "+7"}, "products": [{"id": str(item["id"]), "name": item["name"], "price": item["price"], "quantity": 0}], "total_amount": 0, "payment_method": "card"}))
    rep.test("4.1", "Кол-во=0", "INFO", "нулевое количество", f"статус {r.status_code}", "возможно, запретить")

def t41_large():
    item = get_product()
    r = measure("Кол-во=10000", lambda: create_order("L", item, {"customer": {"name": "L", "email": "l@l.com", "phone": "+7"}, "products": [{"id": str(item["id"]), "name": item["name"], "price": item["price"], "quantity": 10000}], "total_amount": item["price"] * 10000, "payment_method": "card"}))
    if r.status_code == 200 and item["stock"] < 10000: rep.test("4.2", "Кол-во=10000", "WARN", "заказ превышает stock", f"stock={item['stock']}", "проверять остатки")
    else: rep.test("4.2", "Кол-во=10000", "INFO", "большое количество", f"статус {r.status_code}")
...
def t43_long_name():
    item = get_product()
    r = measure("Имя 500", lambda: create_order("A"*500, item)); rep.test("4.4", "Длинное имя", "INFO", "500 символов", f"статус {r.status_code}")


# ═══════════════════════════════════════════════════════════════════════════════
# ║  БЛОК 5. НАГРУЗОЧНОЕ ТЕСТИРОВАНИЕ: скорость при разных нагрузках           ║
# ║  Проверяем: 10 заказов подряд, 5 параллельных заказов                      ║
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
# ║  БЛОК 6. ЮЗАБИЛИТИ: удобство использования API                             ║
# ║  Проверяем: формат ответа JSON, понятность ошибок, технические заголовки   ║
# ═══════════════════════════════════════════════════════════════════════════════
...
def t62_headers():
    r = measure("Заголовки", lambda: requests.get(f"{BASE_URL}{API}/products", timeout=5))
...
# ║  Проверяем: RSS/VMS, запросы/сек, percentiles, устойчивость к всплескам    ║
# ═══════════════════════════════════════════════════════════════════════════════

def p01_memory_baseline():
    proc = psutil.Process(server_proc.pid) if server_proc else None
    if not proc: rep.test("P.1", "Базовая память", "WARN", "процесс не найден"); return
    mem = proc.memory_info()
    rss, vms = mem.rss / 1024 / 1024, mem.vms / 1024 / 1024
    rep.test("P.1", "Базовая память", "PASS", "RSS/VMS сервера", f"RSS: {rss:.1f} МБ, VMS: {vms:.1f} МБ")
    log(f"  RSS: {rss:.1f} МБ | VMS: {vms:.1f} МБ", C.OK, 1)
...
def p02_rps_load(target=50, duration=5):
    ...
    log(f"  {ok}/{total} успешно | Факт RPS: {actual:.1f}", C.OK if st == "OK" else C.WARN, 1)

def p03_memory_load():
    proc = psutil.Process(server_proc.pid) if server_proc else None
    if not proc: rep.test("P.3", "Память под нагрузкой", "WARN", "процесс не найден"); return
    ...
    after = proc.memory_info().rss / 1024 / 1024; delta = after - before
    status = "PASS" if delta < 10 else ("WARN" if delta < 50 else "FAIL")
    rep.test("P.3", "Память под нагрузкой", status, "рост памяти после 50 заказов", f"было: {before:.1f} → стало: {after:.1f} МБ (Δ {delta:+.1f})")
    log(f"  Δ {delta:+.1f} МБ", C.OK if status == "PASS" else C.WARN, 1)
...
# ═══════════════════════════════════════════════════════════════════════════════
# ║  БЛОК 8. ПРИЁМОЧНЫЙ ТЕСТ: полный сценарий покупки от начала до конца       ║
# ║  Проверяем: каталог → товар → заказ → история → статистика                 ║
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
# ║  ЗАПУСК: оркестрация всех блоков                                           ║
# ═══════════════════════════════════════════════════════════════════════════════
...
def run():
    ...
    rep.speed_table(); rep.summary(); rep.save()
    restore(); stop_server()
